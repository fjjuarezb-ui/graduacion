...
.header{background:#0F6E56;border-radius:14px;padding:1rem 1.25rem;margin-bottom:1rem}
.header-title{font-size:16px;font-weight:700;color:#E1F5EE;margin-bottom:2px}
.header-sub{font-size:12px;color:#9FE1CB}
.metrics{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:1rem}
.metric{background:#fff;border-radius:10px;padding:.75rem 1rem}
.metric-label{font-size:12px;color:#666;margin-bottom:2px}
.metric-value{font-size:22px;font-weight:700}
.metric-value.hi{color:#0F6E56}
.prog-bg{background:#ddd;border-radius:99px;height:6px;margin-bottom:3px}
.prog-fill{background:#0F6E56;border-radius:99px;height:6px;transition:width .5s}
.prog-lbl{font-size:11px;color:#888;text-align:right;margin-bottom:1rem}
.card{background:#fff;border-radius:12px;margin-bottom:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.06)}
.card-head{display:flex;align-items:center;gap:10px;padding:.875rem 1rem}
.card-icon{width:36px;height:36px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0}
.card-name{font-size:14px;font-weight:700;flex:1;color:#1a1a1a}
.card-frac{font-size:12px;color:#888;text-align:right}
.card-pct{font-size:15px;font-weight:700;text-align:right}
.card-bar-wrap{padding:0 1rem .75rem}
.card-bar-bg{background:#eee;border-radius:99px;height:5px}
.card-bar-fill{border-radius:99px;height:5px;transition:width .6s}
.bottles{display:flex;flex-wrap:wrap;gap:4px;padding:0 1rem .875rem}
.b{width:28px;height:28px;border-radius:6px;border:1px solid #e0e0e0;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:600;color:#bbb;background:#fff}
.b.open{border-color:transparent;color:white}

.scan-card{background:#fff;border-radius:16px;padding:2rem 1.5rem;text-align:center;margin-top:2rem;box-shadow:0 2px 12px rgba(0,0,0,.08)}
...
.btn-open{background:#0F6E56;color:#fff;border:none;border-radius:10px;padding:16px 0;font-size:16px;font-weight:700;cursor:pointer;width:100%;margin-top:4px}
...
.reset-btn{margin-top:1rem;background:none;border:1px solid #ddd;border-radius:8px;padding:10px;font-size:12px;color:#aaa;cursor:pointer;width:100%}
...
<div id="screen-scan" class="container hidden">
  <div class="scan-card">
    ...
    <div class="scan-sub">Graduación Isabella · <span id="si-date"></span></div>
...
<div id="screen-dash" class="container hidden">
  <div class="header">
    <div class="header-title">🎓 Graduación Isabella Juárez</div>
    <div class="header-sub" id="dash-date"></div>
  </div>
  <div class="metrics">
    <div class="metric"><div class="metric-label">Total botellas</div><div class="metric-value">80</div></div>
    <div class="metric"><div class="metric-label">Abiertas</div><div class="metric-value hi" id="m-open">0</div></div>
    <div class="metric"><div class="metric-label">Disponibles</div><div class="metric-value" id="m-avail">80</div></div>
  ...
  <div class="prog-lbl" id="prog-lbl">0 de 80 abiertas</div>
  <div id="cards"></div>
...
<script>
var LICORES=[
  {name:'Whisky',        em:'🥃', r:[1,15],  c:'#BA7517',bg:'#FAEEDA'},
  {name:'Tequila',       em:'🍹', r:[16,30], c:'#0F6E56',bg:'#E1F5EE'},
  {name:'Ron',           em:'🍺', r:[31,40], c:'#534AB7',bg:'#EEEDFE'},
  {name:'Vodka',         em:'🍸', r:[41,46], c:'#185FA5',bg:'#E6F1FB'},
  {name:'Vino Barefoot', em:'🍷', r:[47,50], c:'#993556',bg:'#FBEAF0'},
  {name:'Prosecco',      em:'🥂', r:[51,80], c:'#3B6D11',bg:'#EAF3DE'},
];
var TOTAL=80,DAYS=7,KEY='grad_v3';

function load(){try{var r=localStorage.getItem(KEY);if(r)return JSON.parse(r);}catch(e){}return null;}
function save(s){try{localStorage.setItem(KEY,JSON.stringify(s));}catch(e){}}
var state=load();
if(!state||!state.created){state={created:Date.now(),opened:{},log:[]};save(state);}

function fmt(ts){try{return new Date(ts).toLocaleString('es-HN',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});}catch(e){return new Date(ts).toLocaleString();}}
function today(){try{return new Date().toLocaleDateString('es-HN',{day:'2-digit',month:'long',year:'numeric'});}catch(e){return new Date().toLocaleDateString();}}
...
function refreshDash(){
  var totalOpen=Object.keys(state.opened).length;
  var pct=Math.round(totalOpen/TOTAL*100);
  document.getElementById('m-open').textContent=totalOpen;
  document.getElementById('m-avail').textContent=TOTAL-totalOpen;
  document.getElementById('m-pct').textContent=pct+'%';
  document.getElementById('prog').style.width=pct+'%';
  document.getElementById('prog-lbl').textContent=totalOpen+' de '+TOTAL+' abiertas';

  var html='';
  for(var i=0;i<LICORES.length;i++){
    var L=LICORES[i];
    var s=L.r[0],e=L.r[1],tot=e-s+1,op=0;
    for(var n=s;n<=e;n++) if(state.opened[n]) op++;
    var lp=Math.round(op/tot*100);
    var cells='';
    for(var n=s;n<=e;n++){
      var open=!!state.opened[n];
      cells+='<div class="b'+(open?' open':'')+'" style="'+(open?'background:'+L.c+';':'')+'" >'+n+'</div>';
    }
    html+='<div class="card">';
    html+='<div class="card-head">';
    html+='<div class="card-icon" style="background:'+L.bg+'">'+L.em+'</div>';
    html+='<div class="card-name">'+L.name+'</div>';
    html+='<div><div class="card-frac">'+op+'/'+tot+'</div><div class="card-pct" style="color:'+L.c+'">'+lp+'%</div></div>';
    html+='</div>';
    html+='<div class="card-bar-wrap"><div class="card-bar-bg"><div class="card-bar-fill" style="width:'+lp+'%;background:'+L.c+'"></div></div></div>';
    html+='<div class="bottles">'+cells+'</div>';
    html+='</div>';
  }
...
function route(){
  ...
  if(b&&t){
    ...
    document.getElementById('si-date').textContent=today();
    if((Date.now()-t)>DAYS*86400000){show('screen-exp');return;}
    var badge=document.getElementById('si-badge');
    var action=document.getElementById('si-action');
    if(state.opened[b]){
      badge.className='badge badge-used';
      badge.textContent='Ya registrada';
      action.innerHTML='<p class="msg-small">Abierta el '+fmt(state.opened[b])+'</p>';
    } else {
  ...
  setInterval(function(){
    var s=load();
    if(s&&JSON.stringify(s.opened)!==JSON.stringify(state.opened)){state=s;refreshDash();}
  },3000);
}
...
[index.html](https://github.com/user-attachments/files/28494225/index-2.html)
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
